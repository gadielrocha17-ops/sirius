...
export const api = {
  ...
  permissions:  { list: () => request('GET', '/permissions'),
                  get: (id) => request('GET', `/permissions/${id}`),
                  create: (b) => request('POST', '/permissions', b),
                  patch: (id, b) => request('PATCH', `/permissions/${id}`, b),
                  delete: (id) => request('DELETE', `/permissions/${id}`),
                  assign: (userId, profileId) => request('PATCH', `/permissions/assign/${userId}`, { profile_id: profileId }) },

  items:        { list: (q) => request('GET', `/items?${new URLSearchParams(q || {})}`),
                  get: (id) => request('GET', `/items/${id}`),
                  movements: (id) => request('GET', `/items/${id}/movements`),
                  create: (b) => request('POST', '/items', b),
                  patch: (id, b) => request('PATCH', `/items/${id}`, b),
                  delete: (id) => request('DELETE', `/items/${id}`),
                  baixa: (id, b) => request('POST', `/items/${id}/baixa`, b),
                  entrada: (id, b) => request('POST', `/items/${id}/entrada`, b) },

  superadmin:   { stats: () => request('GET', '/superadmin/stats'),
                  tenants: () => request('GET', '/superadmin/tenants'),
                  getTenant: (id) => request('GET', `/superadmin/tenants/${id}`),
                  createTenant: (b) => request('POST', '/superadmin/tenants', b),
                  patchTenant: (id, b) => request('PATCH', `/superadmin/tenants/${id}`, b),
                  deleteTenant: (id) => request('DELETE', `/superadmin/tenants/${id}`),
                  createUser: (tenantId, b) => request('POST', `/superadmin/tenants/${tenantId}/users`, b),
                  promote: (userId, b) => request('PATCH', `/superadmin/users/${userId}/promote`, b) },
}
